...
source ./scripts/network/vars.sh
...
do
    ...
    $BINARY init $MONIKER --chain-id $CHAIN_ID --home ${NODE_DIR} --default-denom stake
...
do
    ...
    sed -i.bak'' 's/minimum-gas-prices = ""/minimum-gas-prices = "0.025stake"/' ${NODE_DIR}/config/app.toml

    # Enable Vote Extensions for H-1
    jq '.consensus.params.abci.vote_extensions_enable_height = "1"' ${NODE_DIR}/config/genesis.json > output.json && mv output.json ${NODE_DIR}/config/genesis.json
